...
/* ---------------- state ---------------- */
export const readState = () => kvGet(KEY);

export async function writeState(incomingRev, data) {
  const prev = await readState();
  const rev = Math.max(prev?.rev || 0, Number(incomingRev) || 0) + 1;
  await kvSet(KEY, { rev, data, savedAt: Date.now() });
  return { rev, prev };
}
...
/* ---------------- mail ----------------
   Two senders are supported so you are not blocked on DNS:
     RESEND_API_KEY  → Resend. Best deliverability, but needs a domain you can
                       add DNS records to.
     BREVO_API_KEY   → Brevo. Verify a single "from" address by clicking a link
                       in your inbox — no domain, no DNS.
   Whichever key is present is used; Resend wins if both are set. */
export async function sendMail(portal, o) {
  ...
  try {
    ...
    return { ok: false, error: 'No mail provider configured — set RESEND_API_KEY or BREVO_API_KEY' };
  } catch (e) {
