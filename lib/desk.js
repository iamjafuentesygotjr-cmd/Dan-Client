...
const NLOG = 'desk:v2:notifylog';   // last few notification outcomes, for diagnosis
...
export async function notifyNewMessages(prevData, data) {
  ...
  await recordOutcome(out, fresh.length);
  return out;
}

/* Keep the last few outcomes where they can be read back, so "no email arrived"
   can be answered with the actual reason instead of a guess. */
async function recordOutcome(out, freshCount) {
  try {
    const log = (await kvGet(NLOG)) || [];
    log.unshift({ at: new Date().toISOString(), newMessages: freshCount, results: out });
    await kvSet(NLOG, log.slice(0, 10));
  } catch {}
}
export const readNotifyLog = () => kvGet(NLOG);

/* Try to log in to the mail provider without sending anything. */
export async function verifyMail() {
  const gmailUser = (process.env.GMAIL_USER || '').trim();
  const gmailPass = (process.env.GMAIL_APP_PASSWORD || '').replace(/\s+/g, '');
  if (!gmailUser || !gmailPass) return { checked: false, reason: 'not using Gmail' };
  try {
    ...
    return { checked: true, ok: true, user: gmailUser.replace(/^(.{2}).*(@.*)$/, '$1***$2') };
  } catch (e) {
    return { checked: true, ok: false, error: e.message, code: e.code || null };
  }
}
